...
async function assertAdmin(request) {
  ...
  if (adminData && (adminData.role === "admin" || adminData.admin === true)) {
    return uid;
  }
...
exports.approveUser = onCall(async (request) => {
  ...
  if (!licenceNumber) {
    throw new HttpsError("invalid-argument", "Verified licenceNumber is required.");
  }
  ...
  await userRef.set({
    ...
    freeInspectionsStartedAt: admin.firestore.FieldValue.serverTimestamp(),
...
exports.cancelSubscriptionAtPeriodEnd = onCall(async (request) => {
  ...
  // Stripe is intentionally not implemented in this build.
  // When Stripe is connected, use the Stripe secret key only inside this function
  // and cancel the subscription at period end.
  await db.collection("users").doc(uid).set({
...
exports.deleteMyAccount = onCall(async (request) => {
  ...
  // cancel the Stripe subscription/customer here using secret keys.
