...
var firebaseBillingMessage = "Access status not checked yet.";
...
function getBillingAccessState(profile) {
  var p = profile || {};
  var status = cleanText(p.subscriptionStatus || p.billingStatus || "").toLowerCase();
  var periodEndsAt = p.currentPeriodEndsAt || p.cancelAccessEndsAt || null;

  var freeLimit = Number(p.freeInspectionLimit || 3);
  var freeUsed = Number(p.freeInspectionsUsed || 0);
  ...
  if (!status || status === "not_started" || status === "pending_verification") {
    return {
      ...
      message: "Pending inspector verification. Your first 3 inspections unlock after approval."
    };
  }
  ...
  if (status === "free_inspections") {
    return {
      active: true,
      status: "free_inspections",
      freeRemaining: freeRemaining,
      message: freeRemaining > 0
        ? freeRemaining + " free inspection" + (freeRemaining === 1 ? "" : "s") + " remaining."
        : "You’ve used your 3 free inspections. Subscribe to create more inspections."
    };
  }
  ...
  return {
    ...
    status: status,
    freeRemaining: freeRemaining,
    message: "A subscription is required to continue using BarrierCheck."
  };
}


function hasPaidSubscription(profile) {
  var status = cleanText(profile && profile.subscriptionStatus).toLowerCase();
  return status === "active";
}

function getFreeInspectionsRemaining(profile) {
  var limit = Number((profile && profile.freeInspectionLimit) || 3);
  var used = Number((profile && profile.freeInspectionsUsed) || 0);
  return Math.max(0, limit - used);
}

function showSubscribePrompt() {
  var message = "You’ve used your 3 free inspections.\n\nSubscribe to continue using BarrierCheck for A$20/month.";
...
function startStripeCheckout() {
  callFirebaseFunction("createCheckoutSession", {})
    .then(function (data) {
      if (data && data.url) {
...
function incrementFreeInspectionUsageIfNeeded() {
  if (!firebaseDb || !firebaseUser || hasPaidSubscription(currentUserProfile)) {
    return Promise.resolve();
  }

  var status = cleanText(currentUserProfile && currentUserProfile.subscriptionStatus).toLowerCase();
  if (status !== "free_inspections") {
    return Promise.resolve();
  }

  return firebaseDb.collection("users").doc(firebaseUser.uid).set({
    freeInspectionsUsed: firebase.firestore.FieldValue.increment(1),
    updatedAt: firebase.firestore.FieldValue.serverTimestamp()
  }, { merge: true }).then(function () {
    currentUserProfile.freeInspectionsUsed = Number(currentUserProfile.freeInspectionsUsed || 0) + 1;
    firebaseBillingState = getBillingAccessState(currentUserProfile);
    firebaseBillingActive = !!firebaseBillingState.active;
    firebaseBillingMessage = firebaseBillingState.message;
    updateAuthUI();
  });
}
...
function updateSettingsDisplay() {
  ...
  if (subscriptionText) subscriptionText.textContent = billing.message || "Access status unavailable.";
...
function startNewInspection() {
  ...
  if (!hasPaidSubscription(currentUserProfile) && getFreeInspectionsRemaining(currentUserProfile) <= 0) {
    showSubscribePrompt();
    return;
  }
  ...
  incrementFreeInspectionUsageIfNeeded();
  ...
  if (firebaseEnabled && firebaseUser && firebaseApproved) {
    ...
    if (firebaseDataLoaded) {
      if (!profileCompleted) {
        ...
        statusMessage = firebaseBillingMessage || "Subscription required before more inspections can start";
        isError = true;
      } else {
